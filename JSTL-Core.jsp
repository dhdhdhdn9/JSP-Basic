...
<body>
	...
	<c:set var = "grd" value="가" />
		<!-- gt: = / ge: >=  -->	
		<c:if test="${avg ge 60}"><c:set var = "grd" value="양"/></c:if>
		<c:if test="${avg ge 70}"><c:set var = "grd" value="미"/></c:if>
		<c:if test="${avg ge 80}"><c:set var = "grd" value="우"/></c:if>
		<c:if test="${avg ge 90}"><c:set var = "grd" value="수"/></c:if>
	<c:out value="${grd}" />
